...
public class AutoCaption {
	...
	private String generateBriefDate() {
		...
		briefString = SimpleResources.getStringValue(mCtx,
				R.array.upper_days_in_week_string_values, diff);
		if (!TextUtils.isEmpty(briefString)) {
			return briefString;
		}
		...
		briefString = SimpleResources.getStringValue(mCtx,
				R.array.upper_days_in_last_week_string_values, diff);
		if (!TextUtils.isEmpty(briefString)) {
			return briefString;
		}
	...
	private void generateDateTime() {
		...
		if (TextUtils.isEmpty(briefString)) {
			mDateTimeString = SimpleResources.getResources(mCtx).getString(
					R.string.date_piece, dateString);
		} else {
			mDateTimeString = SimpleResources.getResources(mCtx).getString(
					R.string.date_piece_with_brief_date, briefString,
	...
	public static String generateSentenceWithDateOnly(Context ctx, Date date) {
		return (new AutoCaption(ctx)).setDate(date)
				.generateSentenceWithDateOnly();
	}

	public static String generateSentenceWithDateEventPlacePerson(Context ctx,
			Date date, String event, Place place, Person[] persons) {
		return (new AutoCaption(ctx)).setDate(date).setEvent(event)
				.setPlace(place).setPersons(persons)
				.generateSentenceWithDateEventPlacePerson();
	}
